...
Valid Browser History
    ...
    ${photo_src_1} =  Get Element Attribute    xpath://div[@class="current-photo"]//img    src
    ${photo_id_1} =  String.Split String From Right    ${photo_src_1}  /    2
    Click Link    xpath://a[@data-tippy-content="Next (keystroke ➡)"]
    Wait For Loading Photo
    ${photo_src_2_4} =  Get Element Attribute    xpath://div[@class="current-photo"]//img    src
    ${photo_id_2_4} =  String.Split String From Right    ${photo_src_2_4}  /    2
    Click Link    xpath://a[@data-tippy-content="Next (keystroke ➡)"]
    Wait For Loading Photo
    ${photo_src_3} =  Get Element Attribute    xpath://div[@class="current-photo"]//img    src
    ...
    Go Back
    Wait For Loading Photo
    ${new_photo_src} =  Get Element Attribute    xpath://div[@class="current-photo"]//img    src
