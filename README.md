...
In the above links, replace YOURTOKEN with your token. Use a Mapbox token with the `styles:read` and `fonts:read` scopes. URL restrictions should not be enforced due to some [limitations](https://docs.mapbox.com/accounts/guides/tokens/#requirements-and-limitations).
