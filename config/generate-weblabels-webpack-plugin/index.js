...
// This is a plugin for webpack >= 4 enabling to generate a Web Labels page
// intended to be consumed by the LibreJS Firefox plugin
...
class GenerateWebLabelsPlugin {
    ...
    apply(compiler) {
        compiler.hooks.done.tap(pluginName, (statsObj) => {
            ...
            // set output folder
            this.weblabelsOutputDir = path.join(
                stats.outputPath,
            ...
            stats.modules.forEach((mod) => {
                ...
                mod.chunks.forEach((chunk) => {
                    ...
                    if (
                        ...
                        Object.hasOwn(this.options.srcReplace, srcFilePath)
                    ) {
            ...
            if (this.outputType === "json") {
                ...
                const weblabelsData = JSON.stringify(
                    this.chunkJsAssetToSrcFiles,
                );
                const weblabelsJsonFile = path.join(
                ...
                const weblabelsPageFile = path.join(
                    this.weblabelsOutputDir,
                    "jslicenses.html",
                );
                ejs.renderFile(
                    path.join(__dirname, "jslicenses.ejs"),
    ...
    findLicenseFile(packageJsonDir) {
        if (!Object.hasOwn(this.packageLicenseFile, packageJsonDir)) {
    ...
    copyLicenseFile(licenseFilePath) {
        ...
        if (licenseFilePath && fs.existsSync(licenseFilePath)) {
            let ext = "";
            // add a .txt extension in order to serve license file with
            // text/plain content type to client browsers
    ...
    parsePackageJson(packageJsonPath) {
        if (!Object.hasOwn(this.packageJsonCache, packageJsonPath)) {
        ...
        try {
            if (spdxLicenseExpression.indexOf("AND") !== -1) {
                this.logger.warn(
                    `The SPDX license expression '${spdxLicenseExpression}' ` +
                        `associated to ${context} contains an AND operator, ` +
                        "this is currently not properly handled and erroneous " +
            ...
            this.logger.warn(
                "Unable to parse the SPDX license expression" +
                    ` '${spdxLicenseExpression}' associated to ${context}.`,
            );
            this.logger.warn(
    ...
    spdxToWebLabelsLicense(spdxLicenceId) {
        ...
        this.logger.warn(
            "Unable to associate the SPDX license identifier" +
                ` '${spdxLicenceId}' to a LibreJS supported license.`,
        );
        this.logger.warn(
            "Some generated JavaScript assets may be blocked by LibreJS due to missing license information.",
        );
    ...
    spdxToWebLabelsLicenses(spdxLicenses) {
        // This method simply extracts all referenced licenses in the SPDX
        // expression regardless of their combinations.
        // TODO: Handle licenses combination properly once LibreJS has a
        //  spec for it.
        let ret = [];
        if (Object.hasOwn(spdxLicenses, "license")) {
            ret.push(this.spdxToWebLabelsLicense(spdxLicenses.license));
        } else if (Object.hasOwn(spdxLicenses, "left")) {
            if (Object.hasOwn(spdxLicenses.left, "license")) {
    ...
    extractLicenseInformation(packageJson) {
        ...
        } else if (Object.hasOwn(packageJson, "licenses")) {
    ...
    copyFileToOutputPath(srcFilePath, ext = "") {
        if (
            this.copiedFiles.has(srcFilePath) ||
            srcFilePath.indexOf("://") !== -1 ||
